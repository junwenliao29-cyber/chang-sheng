...
### 13.1 堂食专属菜单（和外卖共用同一套后台）

- 后台 `admin.html` →「菜单管理」顶部切换到 **🍽️ 堂食专属**，就能用和外卖**完全一样**的编辑器管理堂食专属品类/菜品：
  可新增品类、新增菜品、写描述、上传图片、回车保存、上/下一道、快速查找、拖动排序、↑↓、上下架。
- 堂食专属的品类 id 记录在 Supabase `settings.dinein_only_categories`；这些品类和菜品**只显示在 `/interno/` 堂食链接**，外卖站会自动过滤掉。
- 切回「🍜 外卖菜单」就是原来的外卖菜单；「🥤 饮料价格」页仍用于同一杯饮料的「外卖价 / 堂食价」。
